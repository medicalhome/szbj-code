...
(function($){
	...
	$.widget('ui.multiOpenAccordion', {
		...
		_create: function() {
			...
			$h3.each(function(index){
				...
				if($this.next('div').length!=0){
					$this.addClass(options._classes.h3).prepend('<span class="{class}"></span>'.replace(/{class}/,c._classes.span));
		...
		_generateTabsArrayFromOptions: function(tabOption) {
			alert("_generateTabsArrayFromOptions");
			var tabs = [];
			var self = this;
			var $this = self.element;
